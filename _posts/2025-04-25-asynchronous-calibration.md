...
Typically CT scans are not acquired with a calibration phantom in the scan (i.e., synchronous calibration) and so we have to either perform internal calibration or use asynchronous calibration (i.e., scan the phantom in the same scanner as the patient, but not at the same time). The reason a phantom is not used clinically is because most CT imaging is not quantitative, but rather used for diagnostic purposes. We assume that the best way to have quantitative data is to include a calibration phantom, but is it? Does the inclusion of the dense phantom rods actually lead to beam hardening that alters the resulting BMD? This is hard to know because it'd require scanning a person twice: once with the phantom and once without the phantom (and then scan the phantom separately).
